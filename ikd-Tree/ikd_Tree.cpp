...
void KD_TREE<PointType>::multi_thread_rebuild() {
  ...
  while (!terminated) {
    ...
    if (Rebuild_Ptr !=
      ...
      while (search_mutex_counter !=
             0) {  // 当不为0时，表示别的线程正在占用资源
...
void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE** root, PointType point,
  ...
  bool need_rebuild = allow_rebuild & Criterion_Check((*root));
  if (need_rebuild) Rebuild(root);  // 主线程重建
  if ((*root) != nullptr) (*root)->working_flag = false;
  return;
}
...
void KD_TREE<PointType>::Search(KD_TREE_NODE* root, int k_nearest,
  ...
  if (!root->point_deleted) {  // 若当前节点没有被删除，则进行后续的比较
    ...
    if (dist <= max_dist_sqr &&
      ...
      if (q.size() >= k_nearest) q.pop();  // 首先将堆顶节点删除
      PointType_CMP current_point{root->point, dist};
      q.push(current_point);
    }
  }
  int cur_search_counter;
  float dist_left_node = calc_box_dist(root->left_son_ptr,
                                       point);  // 进行递归运算point仍然为查询点
...
void KD_TREE<PointType>::Update(
    KD_TREE_NODE* root) {  // 更新重建的subtree相关属性（treesize, invalidnum,
                           // range etc.）
  /**
   * 1. 对应论文中的Pullup操作
   * 2.
  ...
  if (left_son_ptr != nullptr &&
      right_son_ptr != nullptr) {  // 左、右孩子都存在
  ...
  if (root == Root_Node &&
    ...
    KD_TREE_NODE* son_ptr = root->left_son_ptr;
    if (son_ptr == nullptr) son_ptr = root->right_son_ptr;
    float tmp_bal = float(son_ptr->TreeSize) /
                    (root->TreeSize -
                     1);  // 计算平衡因子，左/右子树节点数除以子树节点数之和
    root->alpha_del =
